...
const Navbar = ({ logout, user, cart, addtoCart, removeFromCart, clearCart, subTotal }) => {
    ...
    return (
        <div className="flex flex-col md:flex-row md:justify-start md:items-center py-3 shadow-xl sticky z-50 top-0 backdrop-blur-3xl">
            ...
            <div className="flex cart absolute items-center right-0 top-5 mx-4">
                {user?.value && <div onClick={toggleDropdown}>
                    <Icon icon="mingcute:user-4-fill" className="md:text-3xl text-2xl mx-2" />
                </div>}
                ...
                {!user?.value && <Link href='/login'>
                    <button className="bg-pink-600 text-white px-4 py-1 mx-2 rounded-md">Login</button>
                </Link>}
                <IoCart className="md:text-3xl text-2xl" onClick={toggleCart} />
            </div>
            <div ref={ref} className={`w-72 h-[100vh] overflow-y-scroll absolute top-0 right-0 bg-pink-100 p-10 z-50${Object.keys(cart).length !== 0 ? ' block' : ' hidden'}`}>
